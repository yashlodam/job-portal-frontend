 ...
 * Side-by-Side Diff Modal comparing Original Content vs AI Suggestion.
 * Includes [Use Suggestion], [Try Again], and [Cancel] actions.
 */
...
export default function AISuggestionModal() {
  const { aiSuggestion, clearAISuggestion, applyAISuggestion, generateAISummary, currentResume } = useResumeBuilder();
  ...
  const handleTryAgain = () => {
    ...
    generateAISummary({
      ...
      role: currentResume?.personalInfo?.professionalTitle || "Software Engineer",
    });
  };
        ...
        >
          ...
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 px-3.5 py-1 text-xs font-black text-indigo-400 uppercase tracking-widest">
              <Sparkles size={14} className="text-amber-300 animate-pulse" /> AI Content Polish Assistant
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            ...
            <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/10 space-y-2">
              ...
              <p className="text-slate-300 font-mono leading-relaxed font-medium min-h-[100px]">
            ...
            <div className="p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 space-y-2">
              <span className="font-black text-indigo-400 uppercase tracking-wider block flex items-center gap-1.5">
                <Sparkles size={13} className="text-amber-400" /> AI Optimized Suggestion
              </span>
              <p className="text-indigo-100 font-medium leading-relaxed min-h-[100px]">
                "{aiContent}"
              </p>
            </div>
          </div>
